 ...
 * Кнопка каждого слайда ведёт именно в раздел, о котором он рассказывает
 * (см. ctaHref): раньше она была одна на все слайды и всегда открывала
 * корень каталога, из-за чего слайд про круги или пасты обещал одно, а
 * кнопка вела совсем в другое.
 */
type Slide = {
  slug: string
  alt: string
  ...
  lead: string
  /**
   * Куда ведёт кнопка «Смотреть оборудование» с ЭТОГО слайда.
   *
   * Раньше кнопка была одна на все слайды и всегда вела в корень
   * каталога — то есть с кадра про круги или пасты человек всё равно
   * попадал на машинки. Клиент поймал это на скринкасте, листая слайд за
   * слайдом. Первый слайд (весь лайнап целиком) по-прежнему ведёт в
   * корень каталога — там нет одной темы, которую можно было бы выделить
   * честно.
   */
  ctaHref: string
}
...
const SLIDES: Slide[] = [
  {
    ...
    lines: ['Оборудование', 'для полировки', 'автомобиля'],
    dim: [1],
    lead: 'Роторные и эксцентриковые машинки ShineMate для полировки автомобиля — кузова, фар, лакокрасочного покрытия.',
    ctaHref: 'catalog',
  },
  {
    slug: 'hero-da',
      ...
      */}
      ...
      >
        ...
        */}
        ...
        >
          <MagneticButton href={slide.ctaHref}>Смотреть оборудование</MagneticButton>
