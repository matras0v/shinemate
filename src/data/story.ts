...
export type SceneDiagram =
   ...
   */
  | { kind: 'velcro' }
  /* Условный разрез ведра с сепаратором — принцип, а не изделие. */
  | { kind: 'separator' }
  /* Держатель: машинка на кузове против машинки на своём месте. */
  | { kind: 'holder'; machineImage: string; holderImage?: string }
  /* Щёточный против бесщёточного — принцип, а не разрез двигателя. */
  | { kind: 'brushless' }
  /*
...
function machineScenes(p: Product): StoryScene[] {
  ...
  if (/бесщёточн|brushless/i.test(p.kind + p.lead)) {
    scenes.push({
      ...
      body:
        'Разница между щёточным и бесщёточным двигателем — в одной паре трения. У щёточного угольные щётки прижаты к вращающейся части и стираются: это расходник с регламентом замены. У бесщёточного этой пары нет, поэтому обслуживание сводится к чистке инструмента.',
      diagram: { kind: 'brushless' },
    })
  }
...
function accessoryScenes(p: Product): StoryScene[] {
  ...
  if (/держател/i.test(p.kind + p.model)) {
    const machine = bySlug('ep820')
    scenes.push({
      title: 'Машинка не лежит на кузове',
      body:
        'Полировальная машинка, положенная на крыло или бампер, — это риск и для лака, и для самой машинки: круг собирает пыль с пола, кабель попадает под ноги, корпус царапает панель. Держатель даёт инструменту постоянное место в шаге от рабочей зоны.',
      diagram: machine
        ? { kind: 'holder', machineImage: machine.image, holderImage: p.image }
        : undefined,
    })
  }

  /*
   * Мойка и подготовка — это ТРИ разных предмета в одной карточке, и у
   * каждого своя мысль. Раньше все они получали один и тот же текст про
   * сепаратор: на микрофибре это читалось как чужое описание. Ветки
   * ниже смотрят на подзаголовок ВЫБРАННОГО исполнения.
   */
  if (/сепаратор/i.test(p.kind + p.model)) {
    scenes.push({
      title: 'Песок остаётся на дне, а не возвращается на кузов',
      body:
        'Без сепаратора смытая грязь висит в воде, и губка забирает её обратно на панель. Тот же песок под кругом работает как абразив и добавляет ровно те риски, которые потом выводят пастой. Решётка на дне пропускает грязь вниз и не даёт ей подняться при следующем заходе.',
      diagram: { kind: 'separator' },
    })
  } else if (/ведро/i.test(p.kind + p.model)) {
    scenes.push({
      title: 'Два ведра — рабочее и для полоскания',
      body:
        'Стандартная схема мойки: в одном ведре состав, во втором чистая вода. Губка полощется во втором и только потом возвращается в состав — грязь не переносится по кругу. Сепаратор на дне закрывает вторую половину задачи.',
      diagram: { kind: 'separator' },
    })
  } else if (/салфет|микрофибр/i.test(p.kind + p.model)) {
    scenes.push({
      title: 'Чем снимают остатки состава',
      body:
        'После прохода круга на панели остаётся плёнка пасты. Снимают её микрофиброй: мягкое полотно забирает остатки, не оставляя своих следов на свежеобработанном лаке. Под каждый этап берут отдельную салфетку, чтобы снятая паста не переносилась обратно.',
      image: p.image,
    })
  }
  ...
  if (p.variants.length >= 2) {
    ...
    scenes.push({
      ...
      body: group
        ? 'В карточке собраны разные позиции одного рабочего этапа. Выберите нужную — название, описание, характеристики, артикул и РРЦ на первом экране обновятся целиком.'
