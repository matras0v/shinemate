/**
 * Контакты сайта. Сайт — самостоятельный продукт под брендом ShineMate,
 * без публичной привязки к конкретному юрлицу-дилеру.
 */

export const company = {
  // Клиент поправил формулировку на видео: не "представительство", а
  // "официальный дистрибьютор" — компания импортирует и продаёт технику,
  // это и есть точная роль по факту.
  role: 'Официальный дистрибьютор ShineMate в России',
  schedule: 'Пн–Пт 09:00–18:00',
  email: 'shinemate_russia@mail.ru',
  /** Почта, на которую уходит заявка из формы. */
  leadEmail: 'shinemate_russia@mail.ru',
  phones: [
    {
      region: 'Москва',
      display: '+7 961 432-99-99',
      href: 'tel:+79614329999',
      address: 'ул. Рябиновая, 43 к 2',
      /** Проверен вручную: этот номер зарегистрирован в WhatsApp. */
      whatsapp: 'https://wa.me/79614329999',
    },
    {
      region: 'Ростов-на-Дону',
      display: '+7 989 129-37-47',
      href: 'tel:+79891293747',
      address: 'ул. Ерёменко, 45',
      /** Проверен вручную: этот номер в WhatsApp недоступен — кнопки нет. */
      whatsapp: null,
    },
  ],
} as const

export const nav = [
  { href: 'about', label: 'О ShineMate' },
  { href: 'technologies', label: 'Технологии' },
  { href: 'contacts', label: 'Контакты' },
] as const
